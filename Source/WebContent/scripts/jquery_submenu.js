...
*/
if (window.tweak_bb == null || window.tweak_bb.page_id == null) window.tweak_bb = {
  page_id: "#pageList",
  row_element: "li"
};
...
jQuery(function ($)
{
  if (location.href.indexOf("listContent.jsp") > 0)
  {
    ...
    if ($("#menuHTML, #menuImage, #menuHTMLSplash").length == 0)
    { // also #menuImageSplash, but leaving out in case the sub page item doesn't have marker
    ...
    headers.filter(":visible").each(function ()
    {
      ...
      if (itemLink.length == 0) $(this).parents(tweak_bb.row_element).addClass("menuitem " + id);
      else if (itemLink.attr("href").indexOf("launchLink.jsp") > 0 || itemLink.attr("href").indexOf("contentWrapper") > 0 || itemLink.attr("href").indexOf("course_id") < 0)
      { // check this 
        href = itemLink.attr("href");
        if (itemLink.filter("[target]").length) target = itemLink.attr("target");
      }
      // push to array
      intMenuItems.push(
      {
...
function setupMenuLinks(intMenuItems)
{
  ...
  for (var i = 0; i < intMenuItemsLength; i++)
  {
    ...
    menuHTML += "<a href='" + intMenuItems[i].href + "' id='" + intMenuItems[i].id + "' title='" + altText + "' target='" + intMenuItems[i].target + "' class='menu_nav_item_" + i + "'>" + intMenuItems[i].title + "</a> ";
    // set any corresponding image map item. benchmark new image map changes
    if (imageMapLinksExist) imageMapLinks.filter("[alt*='" + intMenuItems[i].title + "']").attr(
    {
...
function addMenuToPage(menuLinksHTML)
{
  ...
  else // image or plain text options: setup html, then add to page
  {
    ...
    if (menuImage.length) menuHTML = addMenuImageWrapper(menuImage, menuHTML);
    // add to page
    jQuery(tweak_bb.page_id + "").before(menuHTML);
    // now go fix up the height of that div
...
function addMenuImageWrapper(image, menuHTML)
{
  ...
  var imagesrc = image.attr('src') || '',
      ...
      imagewidth = image[0].width || 0,
      imageheight = image[0].height || 0;
    
  if (imagewidth + imageheight)
  {
    // everything's good to write
    return "<div id=\"graphicMenu\" style=\"background: url(" + imagesrc + "); width:" + imagewidth + "px; height:" + imageheight + "px;\" class=\"" + imageclass + "\">" + menuHTML + "</div>";
  }
  else
  {
    // image missing dimensions, don't set them now - callback will set them later
    fixGraphicMenuHeight(image);
    return "<div id=\"graphicMenu\" style=\"background: url(" + imagesrc + "); \" class=\"" + imageclass + "\">" + menuHTML + "</div>";
  }
}
...
function setupMenuEvents(menuIDs)
{
  jQuery(menuIDs).find("a, area").click(function (event)
  {
    ...
    if (link.attr("href").indexOf("#") != (link.attr("href").length - 1))
    {
      return true;
    } // use regex: IE fix href includes server
    ...
    if (jQuery("#menuHTMLSplash, #menuImageSplash").length) selectSubPage();
...
function selectSubPage()
{
  ...
  else setupMenuEvents("#intmenu"); // check see if required. don't reset up image map areas as submenu double loading events. original comment: not sure why cloning events isn't working - children of intmenu? / does first item only?
...
function displaySection(menuid)
{
  ...
  if (jQuery(tweak_bb.page_id + " ." + menuid).length == 0) loadMenuPage(menuid);
...
function loadMenuPage(menuid)
{
  ...
  if (jQuery("#loading").length == 0) jQuery("<div id=\"loading\"><ul></ul></div>").appendTo("body").hide();
  if (jQuery("#loading li").length > 0) jQuery("#loading li").remove();
  if (jQuery("#loadingGraphic").length == 0) jQuery(tweak_bb.page_id + "").before("<div id=\"loadingGraphic\"></div>");
...
function retriggerTweaks(menuid)
{
  ...
  // quiz
  if (tweak_bb.setupQuiz) tweak_bb.setupQuiz(jQuery);
  // faq
  if (tweak_bb.faq) tweak_bb.faq(jQuery);
  // custom scripts
  if (window.tweak_customScripts) tweak_customScripts(menuid);
}
  ...
  $("#loading a[href*='lobj-']").each(function ()
  {
    ...
    if ($(this).attr("href").indexOf("/webapps", serverOffset) < 0)
    {
      ...
      else if ($(this).text() == "play")
      {
        ...
        if ($(this).parents("span").find("a.downloadlink").length == 0) $(this).after(" | <a href='" + decoded_path + "' target='_download' class='downloadlink' title='Right-click and save this file to your computer' alt='Right-click and save this file to your computer'>download</a>");
      }
    }
  });
}
