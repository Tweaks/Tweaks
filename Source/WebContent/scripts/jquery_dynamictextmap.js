/* 
  ...
  *      Dynamically shift down rows based on preceding row's position
  ...
  20130304 CCB - quick edit to ensure image is loaded before defining width and height
...
function DynamicTextMap() {
    ...
    if (mapsource.length) {
        ...
        this.applyFormatting = function () {
            var lastBase = 0,
                verticalSpacing = 5; // consider taking from CSS if possible to query before writing map? "#tweakmap div.topic:not(\".moduleHeader\"):eq(0)").css("line-height")
            ...
            jQuery(tweak_bb.page_id + " .mapItemLink").each(function (mapItemCount) {
                ...
                if (!useBlackboardItemColours) {
                    mapItemLink.html(mapItemLink.text());
                }
            ...
            if (!staticPositioning) {
                ...
                if (columns == 2 || (columns == 4 && !horizontalLayout)) {
                    ...
                    var i;
                    for (i = 1; i <= columns; i++) {
                        if (tweakmap.find(".module" + i + " .rowBreak").length) {thisRowCol = 0;}
                        thisRowCol++;
...
// 20130304 CCB - quick hack to ensure image is loaded before we mess with it ('width' it... lololol)
jQuery(function($) {
  ...
  // fire
  tempimg.src = mapsource.attr('src');

});

// 20130304 CCB - things to do after image has actually loaded
function unitMapImageInit() {
  ...
  if (DynamicTextMapInstance.hasCustomStyle()) {
      jQuery(function ($) {
          ...
          if (tweak_bb.display_view) { styleSheetHeaders.parents(tweak_bb.row_element).hide(); }

          var numStylesheets = styleSheets.length;
          if (numStylesheets) {
              var $head = $("head");
              var i;
              for (i = 0; i < numStylesheets; i++) {
      ...
      delayedApply(100);
  } // if no delay applied, positioning out
